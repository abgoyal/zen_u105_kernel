...
int ist30xx_tsp_update_info(void)
{
	...
	tsp->buf.raw = kmalloc(tsp->width * sizeof(tsp->buf.raw[0][0]), GFP_KERNEL);
	tsp->buf.base = kmalloc(tsp->width * sizeof(tsp->buf.base[0][0]),
				GFP_KERNEL);
	tsp->buf.filter = kmalloc(tsp->width * sizeof(tsp->buf.filter[0][0]),
				  GFP_KERNEL);
	if (!tsp->buf.raw || !tsp->buf.base || !tsp->buf.filter)
		return -ENOMEM;

	for (i = 0; i < tsp->width; i++) {
		tsp->buf.raw[i] = kmalloc(tsp->height * sizeof(tsp->buf.raw[0][0]),
					  GFP_KERNEL);
		tsp->buf.base[i] = kmalloc(tsp->height * sizeof(tsp->buf.base[0][0]),
					   GFP_KERNEL);
		tsp->buf.filter[i] = kmalloc(tsp->height * sizeof(tsp->buf.filter[0][0]),
					     GFP_KERNEL);
		if (!tsp->buf.raw[i] || !tsp->buf.base[i] || !tsp->buf.filter[i])
			return -ENOMEM;
	}
