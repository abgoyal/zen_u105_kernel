...
static const UWORD8 g_sbox[256] = {
...
static const UWORD8 g_isbox[256] = {
...
static const UWORD8 g_logtable[256] = {
...
static const UWORD8 g_alogtable[256] = {
...
static const UWORD32 g_idxe[4][4] = {
  { 0, 1, 2, 3 },
  { 1, 2, 3, 0 },
  { 2, 3, 0, 1 },
  { 3, 0, 1, 2 }
...
static const UWORD32 g_idxd[4][4] = {
  { 0, 1, 2, 3 },
  { 3, 0, 1, 2 },
  { 2, 3, 0, 1 },
  { 1, 2, 3, 0 }
...
static UWORD8 xtimes(UWORD8 a);
static UWORD8 mul(UWORD8 a, UWORD8 b);
static void   inv_mix_column(UWORD32 *a, UWORD32 *b);
static void   key_addition_8to32(UWORD8 *txt, UWORD32 *key, UWORD32 *out);
static void   key_addition32(UWORD32 *txt, UWORD32 *key, UWORD32 *out);
static void   key_addition32to8(UWORD32 *txt, UWORD32 *key, UWORD8 *out);
...
void aes_setup(aes_context_t *ctx, UWORD8 keysize, UWORD8 *key)
{
    UWORD32 nk      = 0;
    UWORD32 nr      = 0;
    UWORD32 i       = 0;
    ...
    for(i = nk; i < lastkey; i++)
    {
        ...
        if(i % nk == 0)
        {
            ...
            rcon = (UWORD32)xtimes((UWORD8)(rcon & 0xFF));
        }
        else if(nk > 6 && (i % nk) == 4)
        {
...
void aes_encrypt(aes_context_t *ctx, UWORD8 *plaintext, UWORD8 *ciphertext)
{
    UWORD32 r = 0;
    UWORD32 j = 0;
    UWORD32 e = 0;
    ...
    {
        e = wtxt[j] & 0xFF;
        e |= (wtxt[g_idxe[1][j]]) & (UWORD32)(0xFF << 8);
        e |= (wtxt[g_idxe[2][j]]) & (UWORD32)(0xFF << 16);
        e |= (wtxt[g_idxe[3][j]]) & (UWORD32)(0xFF << 24);
        t[j] = e;
    }
...
void aes_decrypt(aes_context_t *ctx, UWORD8 *ciphertext, UWORD8 *plaintext)
{
    UWORD32 r = 0;
    UWORD32 j = 0;
    UWORD32 e = 0;
    ...
    {
        e = wtxt[j] & 0xFF;
        e |= (wtxt[g_idxd[1][j]]) & (UWORD32)(0xFF << 8);
        e |= (wtxt[g_idxd[2][j]]) & (UWORD32)(0xFF << 16);
        e |= (wtxt[g_idxd[3][j]]) & (UWORD32)(0xFF << 24);
        t[j] = e;
    }
...
void inv_mix_column(UWORD32 *a, UWORD32 *b)
{
  UWORD32 i = 0;
  UWORD32 j = 0;
  UWORD8 c[4][4];
  ...
  for(j = 0; j < 4; j++)
  {
      for(i = 0; i < 4; i++)
      {
          c[j][i] = mul(0xE, (UWORD8)((a[j] >> i * 8) & 0xFF))
                  ^ mul(0xB, (UWORD8)((a[j] >> ((i + 1) % 4) * 8) & 0xFF))
                  ^ mul(0xD, (UWORD8)((a[j] >> ((i + 2) % 4) * 8) & 0xFF))
                  ^ mul(0x9, (UWORD8)((a[j] >> ((i + 3) % 4) * 8) & 0xFF));
    }
  }
...
void key_addition_8to32(UWORD8 *txt, UWORD32 *key, UWORD32 *out)
{
  UWORD32 i   = 0;
  UWORD32 j   = 0;
  UWORD32 val = 0;
...
void key_addition32(UWORD32 *txt, UWORD32 *key, UWORD32 *out)
{
  UWORD32 i = 0;
...
void key_addition32to8(UWORD32 *txt, UWORD32 *key,
                                         UWORD8 *out)
{
  UWORD32 i   = 0;
  UWORD32 j   = 0;
