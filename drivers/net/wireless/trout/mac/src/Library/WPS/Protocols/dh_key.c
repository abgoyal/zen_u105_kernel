...
/*         25 02 2011   Ittiam          Modification done                    */
/*                                                                           */
/*****************************************************************************/
...
void wps_dh_core(long_num_struct_t *gen_ptr,
                 ...
                 UWORD32           *temp_mult_buff)
{
    UWORD32 *dh_sec_ptr, *current_mod_value;
    ...
    dh_sec_ptr = exp_ptr->val_ptr;
    current_word = *dh_sec_ptr++;
	...
	/*************************************************************************/
    /*  for (i=1; i<= total number of valid bits in DH Secret; i++)          */
    /*  {                                                                    */
    /*      G = (GxG) mod p;                                                 */
    /*      if(bi == 1)                                                      */
    /*      {                                                                */
...
/*****************************************************************************/
/*                                                                           */
/*  Function Name    : wps_gen_pub_key                                       */
/*                                                                           */
/*  Description      : This function generates Diffie-Hellman public and     */
/*                     private key                                           */
/*                                                                           */
/*  Inputs           : 1) Pointer to the memory   where DH Public Key        */
/*                        should be stored                                   */
...
/*                                                                           */
/*  Outputs          : Public Key =  of (g^private Key) mod p)               */
/*                                                                           */
...
STATUS_T wps_gen_pub_key(UWORD32 *pub_key_ptr,
                         long_num_struct_t *priv_key_ptr)
{
    UWORD32 *temp_u32_ptr;
    UWORD8  *temp_ptr;
    long_num_struct_t gen, temp_pub_key;
#ifndef BIG_ENDIAN
    UWORD32 temp_u32;
#endif /* BIG_ENDIAN */
    UWORD16 cnt;

    /* Allocate temporary buffer required to store Generator */
    temp_u32_ptr = (UWORD32 *)wps_local_mem_alloc(WPS_DH_PRIME_LEN_BYTE);
    temp_pub_key.val_ptr = temp_u32_ptr;

    if(NULL == (temp_pub_key.val_ptr))
    {
		wps_handle_sys_err_reg(NO_LOCAL_MEM);
        return FAILURE;
    }

	/* Allocate temporary memory that is used in key generation */
	temp_u32_ptr = (UWORD32 *)wps_local_mem_alloc(WPS_DH_PRIME_LEN_BYTE * 2);
    if(NULL == temp_u32_ptr)
    {
        wps_local_mem_free((UWORD8 *)temp_pub_key.val_ptr);
        wps_handle_sys_err_reg(NO_LOCAL_MEM);
        return FAILURE;
    }

    temp_pub_key.length = WPS_DH_PRIME_LEN_WORD32;

    /* Generate private key and store it at priv_key_ptr->val_ptr */
    wps_get_rand_byte_array((UWORD8 *)priv_key_ptr->val_ptr,
                            WPS_PRIVATE_KEY_LEN);
    priv_key_ptr->length  = WPS_PRIVATE_KEY_LEN_WORD32;
    priv_key_ptr->msb_pos = wps_check_msb_pos(priv_key_ptr);

    /* Initialize the generator number. Use the Public Key buffer to store   */
    /* the generator                                                         */
    gen.val_ptr = pub_key_ptr;
    memset((void *)gen.val_ptr, 0, WPS_DH_PRIME_LEN_BYTE);
    gen.val_ptr[0] = WPS_DH_GENERATOR;
    gen.length = WPS_DH_PRIME_LEN_WORD32;
    gen.msb_pos = wps_check_msb_pos(&gen);

    /* Generate Public Key using the generator and DH Secret                 */
    wps_dh_core(&gen, priv_key_ptr, &temp_pub_key, temp_u32_ptr);

    /* Public Key is stored in temp_pub_key structure in UWORD32 format where*/
    /* lowest word is stored at first location. Do the conversion in bytes   */
    /* and store the Public Key in bytes and big Endian format in the buffer */
    /* pointed by pub_key_ptr                                                */
    wps_local_mem_free((UWORD8 *)temp_u32_ptr);

    temp_u32_ptr = temp_pub_key.val_ptr + WPS_DH_PRIME_LEN_WORD32 - 1;

#ifndef BIG_ENDIAN
    temp_ptr = (UWORD8 *)pub_key_ptr;
...
/*                                                                           */
/*  Inputs           : 1) Pointer to the memory where DH remote Public Key   */
/*                        is stored                                          */
...
/*                     = (public key ^private Key) mod p) where p is         */
...
STATUS_T wps_gen_ph_dh_key(UWORD32* rx_pub_key,
                           long_num_struct_t *priv_key_ptr)
{
    UWORD32 *temp_u32_ptr;
    UWORD8* temp_ptr;
    ...
    /* Allocate temporary buffer required to store Generator */
    temp_dh_key.val_ptr = (UWORD32 *)wps_local_mem_alloc(WPS_DH_PRIME_LEN_BYTE);
    ...
    if(NULL == temp_u32_ptr)
    {
        wps_local_mem_free((UWORD8 *)temp_dh_key.val_ptr);
        wps_handle_sys_err_reg(NO_LOCAL_MEM);
        return FAILURE;
    }
    ...
    gen.val_ptr = rx_pub_key;
    gen.length = WPS_DH_PRIME_LEN_WORD32;
    gen.msb_pos = wps_check_msb_pos(&gen);


    /* Generate DH_KEY using received public key and DH private key          */
    wps_dh_core(&gen, priv_key_ptr, &temp_dh_key, temp_u32_ptr);

    /* DH_Key is stored in temp_dh_key structure in UWORD32 format where     */
    /* lowest word is stored at first location. Do the conversion in bytes   */
    /* and store the DH Key in bytes and big Endian format in the buffer     */
    /* pointed by rx_pub_key                                                 */
    wps_local_mem_free((UWORD8 *)temp_u32_ptr);

    temp_u32_ptr = temp_dh_key.val_ptr + WPS_DH_PRIME_LEN_WORD32 -1;

#ifndef BIG_ENDIAN
    temp_ptr = (UWORD8 *)rx_pub_key;
    for(cnt =0; cnt < WPS_DH_PRIME_LEN_WORD32; cnt++)
    {
        temp_u32 = *temp_u32_ptr--;
        *temp_ptr++ = (UWORD8)((temp_u32>>24) & 0xFF);
