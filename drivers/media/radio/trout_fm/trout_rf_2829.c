...
int trout_fm_cfg_rf_reg(void)
{
	...
	//chenq mod from james e-mail 2012-12-22
	if (WRITE_RF_REG(0x400, 0X0011) < 0)
		return -1;

	if (WRITE_RF_REG(0x402, 0X086A) < 0)//0X071C) < 0)
		return -1;	
...
int trout_rf_switch(RF_SWITCH_NAME_E switch_name, RF_SWITCH_MODE_E mode)
{
	u32 reg_data;
