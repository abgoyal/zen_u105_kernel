...
static trout_fm_info_t trout_fm_info;

extern bool Set_Trout_RF_Start(unsigned int mode);
extern bool Set_Trout_RF_Stop(unsigned int mode);
extern int  MxdRfSetFreqHKhzFm(unsigned int freqHKHz);

extern void wifimac_sleep_mutex_lock(unsigned int mode);
extern void wifimac_sleep_mutex_unlock(void);
...
void __trout_fm_get_status(void)
{
	...
	trout_fm_info.freq_seek = (trout_fm_info.freq_seek >> 1) + 2;
...
void __trout_fm_show_status(void)
{
	...
	TROUT_PRINT("fm work: 0x%x, fm iq: 0x%04x",(trout_fm_info.fm_sts >> 20 ) & 0x1, (trout_fm_info.fm_sts >> 16 ) & 0xf);
	TROUT_PRINT("seek failed: %d  rssi: 0x%08x",(trout_fm_info.rssi >> 16 ) & 0x1, (trout_fm_info.rssi & 0xFF));
}
...
{
	//WRITE_REG(COM_REG_RF_SPI_CTRL, 0x5);       //4   select RF ADI mode 

	WRITE_RF_REG( 0x0050, 0x0001);

	WRITE_RF_REG( 0x0109, 0x0840);
	WRITE_RF_REG( 0x0109, 0x0cc0); //0x08c0
	//WRITE_RF_REG( 0x0107, 0x003c);
...
int trout_fm_set_freq(int freq)
{
	...
	while(retry < 5)
	{
		...
		/* Add some delay, because sometimes no audio output when open FM.*/
		if(((trout_fm_info.rssi >> 16 ) & 0x1) == 0)	/*seek success.*/
			break;
		msleep(200);
	}
...
//    Set_Trout_RF_Start(FM_MODE);
/* Comment this function because configed by public interface */
...
*/
	...
	//reg_data = (0x1100) | (0x1083UL<<16); //(0x1008) | (0x1006UL<<16);
	reg_data = (0x1100) | (0x106fUL<<16); //(0x1008) | (0x1006UL<<16);  //ZLF
...
/* Called by FM App.*/
/*
    if(trout_fm_info.freq_seek)
	{
		trout_fm_test(trout_fm_info.freq_seek);
	}
*/
...
int trout_fm_dis(void)
{
	...
	READ_REG(FM_REG_FM_EN, &reg_data);
	reg_data &= ~BIT_31;
	WRITE_REG(FM_REG_FM_EN, reg_data);		//1 disable fm, force fm's fsm to idle state
...
static int trout_fm_wait_int(int time_out)
{
	...
	do {
		...
		{	
			...
			trout_fm_dis(); //chenq add 2013-01-08
			
			return -EAGAIN;
		}

		//if(seek_succes && ((trout_fm_info.rssi >> 16) & 1))
		if(seek_succes)
		{
...
int trout_fm_seek(u16  frequency, u8   seek_dir,
	u32  time_out, u16 *freq_found)
{
	int  ret = -EPERM;
	u16 freq = frequency;
        ulong    jiffies_start      = 0;
        u8       is_timeout= 1;

        TROUT_PRINT("FM seek, freq(%i) dir(%i) timeout(%i).",frequency, seek_dir, time_out);
	...
	while(atomic_read(&trout_fm_info.fm_searching))
	{
		...
		if(ret < 0)
		{
			atomic_cmpxchg(&trout_fm_info.fm_searching, 1, 0);
		        TROUT_PRINT("error:seek timeout.");
                        return ret;
		}
		...
		if(((trout_fm_info.rssi >> 16 ) & 0x1) == 0)
			break;
                 ...
                 is_timeout = time_after(jiffies,jiffies_start + msecs_to_jiffies(time_out));
                 if(is_timeout)
                    {
                      atomic_cmpxchg(&trout_fm_info.fm_searching, 1, 0);
		      TROUT_PRINT("error:seek timeout.");
