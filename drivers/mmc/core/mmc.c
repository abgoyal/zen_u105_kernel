	...
	*new_ext_csd = NULL;
	...
	if (err) {
		...
		*new_ext_csd = NULL;
		...
		ext_csd[EXT_CSD_CARD_TYPE] &= 0x0F;
#ifdef CONFIG_MMC_SDHCI_SC8825
        if (ext_csd[EXT_CSD_REV] > 6)//eMMC 5.0 revision is 7 we unsupport
 ...
 */
static void  remove_powerflag(struct mmc_host *host)
{
	int err = 0 , i = 0 ;
...
extern void sdhci_sprd_base_clk_sel (struct sdhci_host *host, struct mmc_card *card);
	...
	struct mmc_card *oldcard)
{
	...
	if (!oldcard) {
		...
		/* only used hynix */
		sdhci_sprd_base_clk_sel (mmc_priv(host), card);
	#endif
	}
