...
extern void __sprd_debug_work_log(struct cpu_workqueue_struct *worker,
...
static inline void sprd_debug_work_log(struct cpu_workqueue_struct *worker,
				      struct work_struct *work, work_func_t f)
{
	if (unlikely(sprd_debug_level.en.kernel_fault))
...
struct sched_log {
	...
	struct work_log {
		...
		struct cpu_workqueue_struct *worker;
